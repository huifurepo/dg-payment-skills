...
1.3.1 目标：在 1.3.0 接入副驾驶基础上新增本地沙箱入口。先判断开发者处境，再推荐产品线、接入方案、本地沙箱演练、参数/返回/代码片段检查、问题修复建议和上线检查。默认只读取当前场景必需的 3-5 份本地 reference。
...
| 本地沙箱通过后准备官方联调或上线 | `references/shared-local-sandbox.md`、`references/copilot-go-live-checklist.md`、`references/official-service-source-index.md` |
...
6. 本地沙箱问题直达 `references/shared-local-sandbox.md`；只有涉及真实联调、公开上线或官方能力边界时才补上线检查和官方资料源索引。
7. 最小复测 prompts 见 canonical regression 文档；完整回归仍使用仓库级脚本。
...
11. 本地沙箱只用于本地协议模拟、闭环演练和自检报告；不得把 local-sandbox 通过解释为官方联调通过、生产准入通过或真实通道/风控/资金结果已验证。
12. 本地沙箱安装或启动类问题，默认引导用户下载构建后的 preview 总包 `https://cloudpnrcdn.oss-cn-shanghai.aliyuncs.com/huifuskills/hf-payment-local-sandbox-latest-preview.zip`，再按系统选择 `dist/` 里的平台 archive 或启动器；不要让普通客户使用仓库本地编译二进制，也不要要求普通客户安装 Go 后自行构建。
...
- 下载与运行：安装/启动类回答必须优先给构建后的 preview 总包 URL，并说明解压后按 Windows、macOS Intel、macOS Apple Silicon 或 Linux 选择 `dist/` 内平台包；不得把 Skill 包或仓库工作区里的本地二进制当作客户分发方式。
- 来源字段：使用 Skill 开发且接入本地沙箱服务时，`HUIFU_SKILL_SOURCE` / `skill_source` / `jpt_x_skill_source` 必须配置为 `hfps/1.3.1;sandbox/1.0.0`；官方联调或生产环境恢复为 `hfps/1.3.1`，不要携带 `;sandbox/...` 后缀。
...
- `HUIFU_SKILL_SOURCE` 最终上送值保持 `<skill_source>` 原样透传，不追加 `sys_id`；本地沙箱模式下由业务配置显式传入 `hfps/1.3.1;sandbox/1.0.0`，生产和官方联调只传 `hfps/1.3.1`。
...
| --- | --- |
| Skill 包版本 | `1.3.1` |
| Skill 包发布日期 | `2026-06-29` |
| 1.3.1 local-sandbox refresh | 新增本地沙箱入口、使用边界、报告口径和上线前证据衔接 |
