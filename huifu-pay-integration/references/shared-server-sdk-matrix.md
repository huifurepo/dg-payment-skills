...
| Python | pip `dg-sdk` | Python 3.x；本地 SDK classifier 只列到 3.7，实际项目需安装验证 | 聚合支付和托管支付核心场景已覆盖，当前 Skill 基线为 `dg-sdk 2.0.21` |
...
当前 Skill 包的 Python SDK 包基线是 `2.0.21`。输出可运行代码前必须验证：
...
- 托管支付 Python 已覆盖预下单、查询、关单、退款、退款查询、对账
