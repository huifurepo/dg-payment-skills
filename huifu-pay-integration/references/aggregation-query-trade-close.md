...
| `merge_flag` | String | 2 | N | `Y`=合单交易关单，`N` 或空=非合单交易关单；为 `Y` 时上送主单信息，其他情况上送子单信息 |

### 原交易定位规则

- `org_req_date` 必填。
- `org_hf_seq_id`、`org_req_seq_id` 不能同时为空。
- 合单交易关单必须显式传 `merge_flag=Y` 并使用主单定位信息；非合单或空值按子单定位。
...
- `resp_code=00000100` 或 `trans_stat=P`：关单处理中，应继续调用 `references/aggregation-query-close-query.md` 确认结果。
