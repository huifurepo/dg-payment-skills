...
└─ 商户自有页面嵌入 checkout 或单支付按钮
   └─ 先接 hostingpay-preorder，再读 checkout-js，最后回到 hostingpay-query 做最终确认
```

## 推荐阅读顺序

```text
1. 先读共享层：签名、`notify_url`、控台 Webhook、请求头、语言矩阵
2. 再读对应产品线 base 文档
3. 再读具体业务场景文档：下单 / 预下单 / 查询 / 退款 / 前端组件
4. 最后把官方 SDK、环境变量和业务真实参数接到项目代码里
```
