...
| 字段 / 配置 | 来源 | 说明 |
|------------|------|------|
| `hosting_data.project_id` | 客户在合作伙伴控台项目管理里创建的统一收银台托管项目 | 必填，项目创建成功后需留存；模型无法猜测 |
...
| 微信授权域名配置 | 微信支付场景的控台 / 微信侧配置 | 官方 H5 / PC 产品文档要求微信支付使用前先配置 `api.huifu.com/hostingH5/`；授权目录/域名不匹配会导致 URL 未注册或 `redirect_uri` 域名不一致 |
...
| `wx_data.sub_appid` | 微信侧应用配置 | 子商户应用 ID，不是模型可推导值；必须已与商户绑定，配置值不要带首尾空格 |
| `wx_data.sub_openid` | 前端登录或授权结果 | 公众号 / 小程序支付场景是关键运行时值，必须从同一个 `sub_appid` 的授权流程获取 |
...
| 接口权限 | 当前托管接口路径 | `接口权限认证失败` 或 `20003` 时先核对 `sys_id` 的接口权限 |
| 数据权限 | `product_id`、`sys_id`、`huifu_id`、`upper_huifu_id` | `数据权限认证失败` 时核对服务商/子商户层级和请求头来源 |
| 通道和场景配置 | 支付产品、费率、线上/线下场景、`pay_channel`/`pay_scene` | 路由无可用通道时先查开通和场景，不要靠生成代码绕过 |
