...
本文件定义当前 Skill 包在 `1.2.0` 版本下的统一请求头约束。
...
无论是官方 Java SDK、官方 PHP SDK，还是自行封装 HTTP 请求，都必须补齐与官方 SDK 一致的请求头。
...
如果业务方显式配置 `HUIFU_SKILL_SOURCE` 或 `skill_source`，HTTP 层应原样透传该值。聚合支付里的 `sys_id` 由各官方 SDK 按自己的实现上送：Java Lightning SDK 会放到独立请求头 `sys_id` / `jpt-sys_id`，PHP 官方 SDK 则仍放在签名前请求体顶层字段。
...
- 当前 Skill 包对齐的官方 PHP SDK 主链路会在 `MerConfig.skill_source` 非空时，于 `BsPay::post()` 主链路自动补 `jpt-x-skill-source`
- 如果当前 PHP 请求参数里的 `huifu_id` 存在且非空，且 `MerConfig.skill_source` 已配置，SDK 还会自动补 `jpt-x-skill-huifu_id: <data.huifu_id>`
- `BsPaySdk\core\Payment` 和 `BsPayClient::postRequest()` 都走这条官方 `BsPay::post()` 主链路，不需要再为来源头打补丁
- PHP 侧只有在绕开官方 SDK 主链路时，才需要手动补 `jpt-x-skill-source` / `jpt-x-skill-huifu_id`

源码核对提示：

- 当前本地 PHP SDK 在补 `jpt-x-skill-huifu_id` 前直接读取 `$request_params['huifu_id']`；如果某接口请求参数没有 `huifu_id` 且运行环境把 PHP warning 升级为异常，应按 SDK 源码修正或升级 SDK，不要通过关闭 warning、吞异常或写固定 `huifu_id` 来掩盖问题
...
`sdk_version` 是 SDK 自动生成的 HTTP 请求头，不作为手写值或安装包下载依据；当前 Skill 的 PHP 安装包基线见 `references/shared-server-sdk-matrix.md`，为 `huifurepo/dg-php-sdk 2.0.26`。
...
- `sys_id` 仍由 SDK 放入签名前请求体顶层字段，不要误写成 PHP 请求头模板
- `sdk_version` 是 SDK 自动生成的 HTTP 请求头，不作为手写值或安装包下载依据；当前 Skill 的 PHP 安装包基线见 `references/shared-server-sdk-matrix.md`，为 `huifurepo/dg-php-sdk 2.0.26`

## 脚本语言约束

先区分两类情况，不要把 PHP 官方 SDK 路径和“没有自动补头能力的脚本语言”混为一谈。
...
如果接入方使用 Python、Go、Node.js 等没有官方 SDK 自动补头能力的脚本语言，或者 PHP 虽然是脚本语言但当前绕开了官方 `BsPay::post()` 主链路，就必须在 HTTP 客户端层手动补齐上述来源头。

手动补头时只对齐 PHP 官方 SDK 请求头口径：`sdk_version: php#v2.0.26`、`jpt-x-skill-source` 和条件性的 `jpt-x-skill-huifu_id`。聚合支付 PHP 路径也不要生成 Java Lightning 专属的 `jpt-sdk_version`、`sys_id`、`jpt-sys_id` 请求头。

## 常规 HTTP 与报文约束

下面这些是基础 HTTP / 报文约束，不替代上面的 SDK 对齐头：
