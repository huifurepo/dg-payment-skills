...
- 产品能力主线：聚合支付、托管支付、前端支付组件、商户进件、抖音直连、拆单支付查询
- 1.3.2 Copilot 能力：首次接入、存量/疑似存量识别、商户进件 API 路由与完整字段目录、联调方案卡、FAQ 排查、参数检查、本地沙箱入口、上线检查和技术支持升级
...
| 企业/个人进件、图片上传、业务开通、详情与申请状态 | 六个原子 reference 加 `references/merchant-onboarding-complete-field-catalog.md`、`references/merchant-onboarding-field-contracts.md`、`references/merchant-onboarding-external-resources.md` | 精确 API 路由、六接口完整字段路径、高风险合同和字段级外部资料提示；命中链接/XLSX/固定协议时给完整路径与原始地址，示例 URL 不作默认值；仅 `file_url` 有按语言区分的受控 SDK 片段，本地文件流和审核/电子协议回调不生成实现；逐业务通知仅按官网 `ord_id` 说明 ACK |
...
| Python | `references/shared-server-sdk-matrix.md` | 聚合支付核心主链路和托管支付核心场景已覆盖，当前 Skill 基线为 `dg-sdk 2.0.23`，生产环境变量清单见 Python adapter 文档 |
...
本地沙箱 `hf-payment-local-sandbox` 用于正式联调前的本地协议模拟、闭环演练、故障注入和自检报告。当前源码候选为 `1.0.1`，继续使用未变更的 r4 协议快照；公开 preview 仍是 `1.0.0`。仓库不把沙箱二进制或内部 RC 制品放入 Skill 包，也不新增商户进件模拟端点。
...
| --- | --- |
| Skill 包版本 | `1.3.2` |
| 1.3.2 merchant onboarding field refresh | `2026-07-17`，新增六接口 1393 条完整字段路径、外部资料索引和图片上传 SDK 安全边界 |
...
| 本地沙箱源码候选 | `hf-payment-local-sandbox 1.0.1`，contract bundle `huifu-pay-integration-1.3.0-r4`，report schema `1.8`；当前公开 preview 仍为 `1.0.0`，不得宣称 `1.0.1` 已发布 |
...
| 商户进件 SDK 证据 | 五个 JSON 接口三语言均有可选 Request 类；图片 `file_url` 模式按 Java multipart 文本字段、PHP/Python JSON 分别处理，本地文件流继续阻断 |
...
SDK 源码证据校验需要把已核验 SDK 根目录显式传给门禁；该 SDK 不会打入 Skill：

```bash
HUIFU_SDK_ROOT=/path/to/sdk scripts/validate-skills.sh
```

发布门禁使用强制 SDK 证据入口：

```bash
HUIFU_SDK_ROOT=/path/to/sdk scripts/validate-release.sh
```
...
正式证据运行必须基于已提交且相关源文件干净的提交，并使用全新的空输出目录；脚本会拒绝在旧目录续跑，防止不同提交或不同批次的结果混入。每次新运行都会生成 `run-manifest.json`，记录提交 SHA、源文件 SHA-256、隔离工作区 SHA-256、模型、case 清单、逐 case 输出 SHA-256 和最终通过数。`--skip-run` 只允许重评由当前同一提交、同一 evaluator 生成且输出哈希完整的目录；缺少清单、源不一致、case 混批或输出被改动都会拒绝。该模式仍只用于诊断，不能替代固定提交的一次性正式运行。

真实模型回归 smoke 当前为 25 条，以 `run_real_model_regression.py` 的 `SMOKE_CASE_IDS` 为准，覆盖首次接入、硬检查点、SDK 落地、禁绕验签、上线检查、字段保留、商户进件边界、完整字段路径和字段级外部资料提示；随 Skill 分发的 `references/canonical-regression-prompts.md` 是独立的 8 条最小回归 Prompt，两者为不同集合。发布验收必须设置 `HUIFU_SDK_ROOT`；未设置时的静态运行仅是本地快检。
...
- 1.3.2 在接入副驾驶层上新增商户进件完整字段目录、外部资料索引和 SDK 安全边界；普通接入仍优先从 `references/copilot-onboarding.md` 和 `references/copilot-solution-selection.md` 判断场景
...
- 已暴露的私钥必须立即吊销/轮换；提交前执行 `scripts/scan_sensitive_examples.py`，只允许占位符示例
