...
这是一个面向第三方客户的 Huifu 接入 Skill 包，用来帮助开发者借助 AI 工具完成汇付支付 / 斗拱 SDK 接入开发、联调排查和上线检查。
...
- 1.3.0 Copilot 能力：首次接入、存量/疑似存量识别、存量系统增量接入、联调方案卡、问题解决中心、FAQ 排查、参数检查、上线检查和技术支持升级
...
Windows 或 CI 不能直接解析 `codex` 时，通过 `--codex-executable` 或 `CODEX_EXECUTABLE` 指定可执行文件；正式发版记录应写明 smoke / full 实际用例数、通过数、模型和输出目录，全量用例数以 `scripts/huifu-payment-test-prompts.json` 为准。
...
- 1.3.0 新增接入副驾驶层，优先从 `references/copilot-onboarding.md` 和 `references/copilot-solution-selection.md` 判断场景
